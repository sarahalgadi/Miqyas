...
//-------------------------DID NOT TEST YET but the logic for comparing weights against activity weight is here -----------------
  async function saveGrades(req, res){
      ...
      try{
        ...
        // Extract user-inputted weights from the form
        ...
        // Extract saved weights and calculate their total
        const totalSavedWeights = directAssessment.reduce((total, item) => total + item.weight, 0);

        // Compare the sum of user-inputted weights with the total saved weights
        if (sumOfUserWeights !== totalSavedWeights) {
            // If they don't match, notify user 
            res.send('<script>alert("Weights added do not sum up to the total expected weight of assessment activity"); window.location.href = "/grades/' + courseCode + '/' + term + '/' + section + '";</script>');
        }
